...
// Command codes:
const COL_ADDR_SET: i8 = 0x2A;
const ROW_ADDR_SET: i8 = 0x2B;
const MEM_WR: i8 = 0x2C;
const COLOR_MODE: i8 = 0x3A;
const COLOR_MODE__12_BIT: i8 = 0x03;
const COLOR_MODE__16_BIT: i8 = 0x05;
const COLOR_MODE__18_BIT: i8 = 0x06;
const MEM_WR_CONT: i8 = 0x3C;


pub fn GC9A01_delay(ms: u16) {
    delay(ms);
}

pub fn GC9A01_set_reset(val: i8) {
    digitalWrite(RES, val);
}

pub fn GC9A01_set_data_command(val: i8) {
    digitalWrite(DC, val);
}

pub fn GC9A01_set_chip_select(val: i8) {
    digitalWrite(CS, val);
}

pub fn GC9A01_spi_tx(data: &[i8]) {
...
pub fn GC9A01_write_command(cmd: &[i8]) {
...
pub fn GC9A01_write_data(data: &[i8]) {
...
pub fn GC9A01_write_byte(val: i8) {
...
pub fn GC9A01_set_frame(/*struct GC9A01_frame frame*/) {
...
pub fn GC9A01_write(data: i8) {
    // data needs to be a vector
    GC9A01_write_command(MEM_WR);
    GC9A01_write_data(data, len);
}

pub fn GC9A01_write_continue(data: i8) {
    // data needs to be a vector
